...
import type { IndexedItem } from "@/lib/search";
...
const CATEGORY_LABELS: Record<string, string> = {
  outfits: "Tools", beauty: "Parts", toiletries: "Vehicles", essentials: "Storage",
};
...
function SearchResults({
  ...
  onGroupClick: (outfit: SavedOutfit) => void;
}) {
            ...
            {items.map((item) => (
              <button
                ...
                className="flex items-center gap-3 bg-white border-2 border-black rounded-xl
                           px-3 py-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]
                  ...
                  <div className="flex gap-1">
                    {[0, 1, 2].map((i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="w-10 h-10 border-2 border-black rounded-lg overflow-hidden flex-shrink-0"
                          style={{ background: "#F2F2F2" }}
                        >
                          {it?.imageObjectPath ? (
...
export default function SavedPage() {
  ...
  // Scroll to top the moment the user starts typing
  useEffect(() => {
    if (searchQuery) pageTopRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [searchQuery !== ""]);
  ...
  const searchResults = isSearching
    ? searchAll(allItems as IndexedItem[], outfits ?? [], searchQuery)
    : null;
    ...
    <div className="min-h-full flex flex-col pt-8 px-4 pb-8 relative" style={{ background: "#F2F2F2" }}>
      ...
      <div className="mb-5 relative">
        ...
        <input
          ...
          className="w-full h-11 pl-9 pr-9 border-2 border-black rounded-xl text-sm font-medium
                     bg-white focus:outline-none focus:ring-2 focus:ring-primary
                     placeholder:font-normal placeholder:text-black/30
      ...
      {isSearching && searchResults ? (
        <SearchResults
          ...
          onItemClick={(item) => setDetailsItem(item)}
          onGroupClick={() => setSearchQuery("")}
        />
      ) : (
        <>
          {atLimit && !isLoading && (
                              ...
                              {item ? (
                                <>
                                  ...
                                  <div className="flex items-center justify-between px-0.5">
                                    <span className="text-[8px] font-bold uppercase text-muted-foreground truncate">{SLOT_LABELS[slot]}</span>
                                    <button
                                ...
                                <>
                                  ...
                                  <span className="text-[8px] font-bold uppercase text-black/25 text-center truncate">{SLOT_LABELS[slot]}</span>
                                </>
                              )}
                            </div>
                          );
      ...
      {/* Item details — showAddToLookbook because this is the Lookbook/Saved context */}
      <AnimatePresence>
        {detailsItem && (
          <ItemDetailsSheet
            key={detailsItem.id}
